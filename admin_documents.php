...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($source === 'admin') {
        ...
        if ($res && $row = $res->fetch_assoc()) {
            ...
            $stmt->bind_param('iisis', $id, $_SESSION['user_id'] ?? 1, $user_name, $row['case_id'], $row['file_name']);
            $stmt->execute();
        ...
        if ($res && $row = $res->fetch_assoc()) {
            ...
            $stmt->bind_param('iisis', $id, $_SESSION['user_id'] ?? 1, $user_name, $row['form_number'], $row['file_name']);
            $stmt->execute();
