...
// Admin's own cases (if they are also an attorney)
$admin_own_cases = 0;
$admin_own_clients = 0;
if (isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'admin') {
    $admin_own_cases = $conn->query("SELECT COUNT(*) FROM attorney_cases WHERE attorney_id=$admin_id")->fetch_row()[0];
    $admin_own_clients = $conn->query("SELECT COUNT(DISTINCT client_id) FROM attorney_cases WHERE attorney_id=$admin_id AND client_id IS NOT NULL")->fetch_row()[0];
}
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Admin's Attorney Statistics -->
        <div class="dashboard-cards">
            <div class="card">
                <div class="card-title">My Cases</div>
                <div class="card-value"><?= $admin_own_cases ?></div>
                <div class="card-description">Cases I'm handling as attorney</div>
            </div>
            <div class="card">
                <div class="card-title">My Clients</div>
                <div class="card-value"><?= $admin_own_clients ?></div>
                <div class="card-description">Clients I'm representing</div>
            </div>
        </div>
