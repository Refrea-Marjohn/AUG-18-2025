...
// Debug logging
error_log("Update event status request - Event ID: $event_id, New Status: $new_status, User ID: " . ($_SESSION['user_id'] ?? 'not set'));

// Validate status value
$valid_statuses = ['scheduled', 'completed', 'no-show', 'rescheduled', 'cancelled'];
...
try {
    ...
    $stmt = $conn->prepare("UPDATE case_schedules SET status = ? WHERE id = ? AND attorney_id = ?");
    $stmt->bind_param("sii", $new_status, $event_id, $_SESSION['user_id']);
    
    // Debug: Log the update attempt
    error_log("Attempting to update event $event_id to status '$new_status' for attorney " . $_SESSION['user_id']);
